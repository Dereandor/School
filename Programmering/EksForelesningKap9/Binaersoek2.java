...
class Binaersoek2 {
  ...
  public static void main(String[] args) {
    int[] enTabell = {-5, -5, -2, 0, 3, 4, 8, 11, 11, 13, 17, 22, 22,0,0,0,0,0};
   System.out.println("Totalt antall tester: 6");
    int indeks = binærsøkHeltallstabell(enTabell, 13, 22);  // tall finnes
    if (indeks == 11 || indeks == 12) {
      System.out.println("Binaersoek: Test 1 vellykket");
    }
    indeks = binærsøkHeltallstabell(enTabell, 13, 0); // tall finnes
    if (indeks == 3) {
      System.out.println("Binaersoek: Test 2 vellykket");
    }
    indeks = binærsøkHeltallstabell(enTabell, 13, -5); // tall finnes
    if (indeks == 0 || indeks == 1) {
      System.out.println("Binaersoek: Test 3 vellykket");
    }
    indeks = binærsøkHeltallstabell(enTabell, 13, -8); // tall finnes ikke
    if ((-indeks - 1) == 0) {
      System.out.println("Binaersoek: Test 4 vellykket");
    }
    indeks = binærsøkHeltallstabell(enTabell, 13, 5); // tall finnes ikke
    if ((-indeks - 1) == 6) {
      System.out.println("Binaersoek: Test 5 vellykket");
    }
    indeks = binærsøkHeltallstabell(enTabell, 13, 25); // tall finnes ikke
    if ((-indeks - 1) == 13) {
