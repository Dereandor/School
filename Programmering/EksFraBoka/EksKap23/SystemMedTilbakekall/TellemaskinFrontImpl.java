...
class TellemaskinFrontImpl
   ...
   * synkronisert.(ArrayList er ikke synkronisert. Hvis du trenger en synkronisert
   * ArrayList, er det klassen Vector du skal bruke. Se online API-dok.)
   */
  ...
  /* Melder ut en klient. Ingenting skjer hvis klient med dette navn ikke eksisterer. */
  public synchronized void meldMegUt(String klientnavn) throws RemoteException {
    boolean funnet = false;
    int klientIndeks = 0;
    while (klientIndeks < klientene.size() && !funnet) {
      Klient denne = klientene.get(klientIndeks);
      if ((denne.getNavn()).equals(klientnavn)) {
        funnet = true;
        klientene.remove(klientIndeks);
        System.out.println("Nå er klienten " + klientnavn + " fjernet.");
      } else {
        klientIndeks++;
      }
    }
